...
pnpm run ci:restricted      # 受限自动化同款检查,与 ci 覆盖一致
pnpm run daily:automation   # 自动化日报同款检查,与 daily:local 覆盖一致
```

后端与 `@metro-ops/shared` 测试统一使用 `node --import tsx --test`,避免受限环境触发 `tsx --test` 的 IPC 权限问题。`ci` / `ci:restricted` 都执行 `lint + typecheck + pnpm -r test`;`daily:local` / `daily:automation` 都执行同一组 `lint + typecheck + test` 日常检查,不再为 shared tests 开特殊跳过分支。
